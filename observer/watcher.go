...
// the delay is applied after an attempt finishes, not from when it started: a connect to a dead
// host spends seconds in the TCP timeout on its own
...
func (w *watcher) checkSilence() {
	...
	w.event = gen.Event{}
	// a fresh request, not a fresh monitor: the producer terminates when it thinks
	// nobody is listening, and asking again is what brings it back
	w.connect()
}
...
func (w *watcher) connect() {
	// a wake or a stale retry timer must not subscribe twice
	if w.event.Name != "" {
		return
	}
