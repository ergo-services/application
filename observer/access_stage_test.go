...
// a tenant groups subjects, so two deployments that both call somebody admin do not end up
// reading and cancelling each other's runs
func TestTenantsDoNotShareAKeyspace(t *testing.T) {
	...
	// the same key under another tenant is a key of its own, not a collision
...
// the header is believed, so what it says has to reach the ceiling: an absent one is a
// challenge, a group nobody granted is a refusal, and what a group grants bounds every call
func TestTrustedHeaderOverHTTP(t *testing.T) {
	...
	// sre is denied manage.kill by name, viewer by being read-only
...
// a run belongs to the caller the header names: the key addresses it inside that caller, and
// another caller does not see it, cannot read it and cannot cancel it
