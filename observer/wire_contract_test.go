...
// The shape of an answer is decided here, so nothing the framework hands over may decide it
// instead: a type of gen that owns its own json form would change the wire from the outside.
...
// the browser DTO is a hand-written mirror too, and nothing but this keeps it one: a field the
// framework adds, or one somebody flattened and forgot half of, disappears without a word
var wireBrowserSplits = map[string][]string{
	"EventInfo.Event":     {"Name", "Node"},
	"ProcessInfo.Tracing": {"TracingSampler", "TracingAttributes"},
}

// a framework type reused for something that fills only part of it
