...
// a cross-origin mutation is refused before it runs
...
// a caller that cannot take gzip is answered without it
func TestStaticHonoursAcceptEncoding(t *testing.T) {
	page := []byte("<html>observer</html>")
	packed := &bytes.Buffer{}
	...
	for _, c := range []struct {
		...
		{"a browser", "gzip, deflate, br", true},
		{"a wildcard", "*", true},
		{"a caller that takes nothing", "", false},
		{"a caller that takes only deflate", "deflate", false},
		{"a caller refusing gzip by weight", "gzip;q=0, deflate", false},
		{"a caller refusing everything else", "gzip;q=1.0, *;q=0", true},
	} {
		recorder := httptest.NewRecorder()
		...
		if recorder.Code != http.StatusOK {
			t.Errorf("%s got %d", c.what, recorder.Code)
			continue
		}
		encoding := recorder.Header().Get("Content-Encoding")
		if (encoding == "gzip") != c.compress {
			t.Errorf("%s: Accept-Encoding %q answered with encoding %q", c.what, c.accept, encoding)
		}
		if strings.Contains(recorder.Header().Get("Vary"), "Accept-Encoding") == false {
			t.Errorf("%s: the answer varies by encoding and does not say so", c.what)
		}
		if c.compress == false && recorder.Body.String() != string(page) {
			t.Errorf("%s: the body is not readable: %q", c.what, recorder.Body.String())
		}
	}
}
