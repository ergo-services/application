...
// the drift guard both ways: a lens added and not announced, and an entry announced that the
// surface cannot read
...
func TestResourceTemplatesExpandToReadableURIs(t *testing.T) {
	...
	for _, entry := range mcpResourceTemplates(Ceiling{}) {
		...
		// the query part is optional: a reader that fills nothing in must still be left with
		// a uri
...
// the conversion is by name, so the names a lens announces and the names the conversion knows
// have to agree
func TestAnnouncedParamsSurviveTheConversion(t *testing.T) {
	for _, spec := range lensSpecs {
		for _, param := range spec.Params {
			if param == "since" {
				// since is a read parameter, not an argument: it never reaches a builder
				continue
			}
...
// comparing each shape against the request built without the parameter says which one the
// builder reads: any other shape is accepted and quietly does nothing
func TestAnnouncedParamsAreReadInTheShapeTheyArriveIn(t *testing.T) {
	// several texts, because a builder may act only on words of its own: a probe of one word
	// would call a parameter unread when it is only unmatched
	...
	for _, spec := range lensSpecs {
		...
		for _, param := range spec.Params {
			...
			if err != nil {
				// the lens needs a target to build at all, and that is not under test here
				continue
			}
