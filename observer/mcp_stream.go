...
// the answer to the listen request goes last: a stream that ends with it ended by our choice,
// while one that simply stops was a connection dropping
...
func (s *mcpStream) raise(uri mcpURI) (gen.Event, error) {
	...
	// without this an owner is raised for a node that is not there, the subscription is
	// acknowledged, and the client holds a slot that will never carry anything
