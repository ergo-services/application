...
// a Go field name cannot hold a slash, so this never collides with a field of the answer
...
// a key is the path from the object carrying the legend, and a step into a collection is
// written [] so that "Processes[].Uptime" cannot be read as a field of this object
...
// a pointer is the same object one level down, the rest are many
...
// one table, keyed by the fact rather than by which struct carried it: the same absence arrives
// as a Response while a subscription is being raised and as a Message once it is
