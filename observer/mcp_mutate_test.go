...
// the receiver reads a Go type, so a message announced as int8 has to arrive as int8 and not as
// the number json happens to carry
func TestSendCarriesTheNamedType(t *testing.T) {
	...
	// an alias asks for the other capability and the other request
	action, out, err := buildSend(map[string]any{
...
// absent means normal, and a meta takes no priority at all: the framework request has no field
// for it
...
func TestSchemaEnumsAreParsed(t *testing.T) {
	// the text form, which is what a listing hands an agent and what the schema asks for
...
// answers with what the tool returned and the error object, so a test can assert either
...
func mcpRead(t *testing.T, base string, uri string, headers map[string]string) map[string]any {
	...
	// a reading is one ceiling's view of one node, so a shared cache would hand it to
	// somebody else
...
// a lens is read from the value its owner holds, and that value moves once per inspector
// period
...
func TestMCPProcessTuneEveryKnob(t *testing.T) {
	...
	// each value is of the type the schema declares for its field: text asserted as a bool
	// would set the opposite
	turns := []struct {
		knob  string
		field string
		value any
	...
	// every one of these differs from what the process was born with: asserting a value that
	// matches the default proves nothing
...
// the system application refuses to stop and start, and stopping the observer would take the
