...
func (s *session) Init(args ...any) error {
	...
	// Init runs inside the manager callback: collecting nodes and monitoring the registrar
	// event here would queue every other connect
...
func (s *session) HandleMessage(from gen.PID, message any) error {
	...
	case gen.MessageDownEvent:
		...
		// the source is already gone, so no DemonitorEvent here
...
func (s *session) HandleCall(from gen.PID, ref gen.Ref, request any) (any, error) {
	...
	case commandRequest:
		...
		result, err := s.handleCommand(r)
		return s.counted("command_refused", result, err)
	case actionRequest:
		if refused := s.notMine(r.Subject); refused != nil {
			return apiResponse{Error: refused.Error()}, nil
		}
		result, err := s.handleAction(r)
...
func (s *session) Terminate(reason error) {
	...
	// LinkAlias is one-way: without this the meta holds a silent stream open
...
func (s *session) handleAction(req actionRequest) (any, error) {
	// self-inspect cannot round-trip: this session would block in CallWithTimeout while
	// system_inspect tries to inspect it, and a process in a Call does not serve HandleInspect
	...
	// the ceiling was checked against the observed node, so an identifier is resolved against
	// it and one naming another node is refused
	...
	callStart := time.Now()
	result, err := s.CallWithTimeout(target, actionReq, defaultCallTimeout)
	...
	return actionResponse(result, s.ceiling), nil
}

// the browser projection: the agent path renders framework types instead
...
func (s *session) clusterInfo() (any, error) {
	target := gen.ProcessID{Name: clusterName, Node: s.Node().Name()}

	result, err := s.CallWithTimeout(target, RequestClusterInfo{}, defaultCallTimeout)
	if err != nil {
	...
	if ok == false {
		return apiResponse{Error: fmt.Sprintf("unexpected response %T", result)}, nil
	}
	return apiResponse{OK: true, Data: wireClusterFrom(info)}, nil
}
...
func buildActionRequest(action string, args map[string]any) (any, string, error) {
	...
	case "send":
		...
		// absent is normal: gen.MessagePriorityNormal is the zero value
	...
	case "heap":
		...
		// no default: the browser shows the whole table, and the surface that needs a page
		// asks for one
	...
	case "processes":
		...
		// two different questions, so two capabilities
...
func (s *session) doSubscribe(subType string, args map[string]any) (any, error) {
	...
	if _, held := s.subIndex[handle]; held == false && len(s.subscriptions) >= s.maxSubscriptions {
		s.dropped["subscription_limit"]++
		s.Log().Warning("session %s: refused %s, %d subscriptions is the limit", s.id, subType, s.maxSubscriptions)
	...
	// the inspector may answer the same scope with another event after a restart
	holds := 0
	if oldEventKey, exist := s.subIndex[handle]; exist {
		if oldEventKey == eventKey {
			holds = s.subscriptions[oldEventKey].handles[handle]
		} else {
			holds = s.dropHandle(handle, oldEventKey)
			s.Log().Info("session %s: auto-unsubscribed %s (replaced)", s.id, oldEventKey)
		}
	}

	// several scopes can resolve to one inspector event: monitor it once, index every handle
	...
	sub.handles[handle] = holds + 1
	s.subIndex[handle] = eventKey
	s.lastChange = time.Now()
	s.Log().Info("session %s: subscribed %s [%s] → %s (total subs: %d)", s.id, subType, handle, eventKey, len(s.subscriptions))

	s.sendInitialData(subType, result)

	return apiResponse{OK: true, Data: wireSubscribed{Key: handle}}, nil
}
...
func buildInspectRequest(subType string, args map[string]any) (any, string, error) {
	...
	case "process_list":
		...
		// -1 asks for the whole table: an unordered scan of the live map
		if lim == -1 {
...
// the session is in the name so a release in flight from a dead one cannot resolve here
...
func subLookupKey(subType string, args map[string]any) string {
	...
	case "event_stream":
		// the full scope, matching the inspector's hash: a stale unsubscribe for one scope
		// must not remove a fresh subscription for another
		if name, ok := args["name"].(string); ok {
			limit := 500
			if v, ok := args["limit"].(float64); ok && v >= 1 {
				limit = int(v)
