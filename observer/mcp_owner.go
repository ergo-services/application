...
// these publish deltas, not state: a missed publication is lost lines, so they are kept in a
// ring and read with a cursor
...
func (o *uriOwner) read(request ownerReadRequest) error {
	...
	// pending first: produce answers it on failure, and an empty pending swallows the reason
	o.pending = append(o.pending, request)
	if err := o.produce(); err != nil {
		return err
	}

	// the inspector publishes only when something happened, so a quiet node never answers:
	// "no errors in the log" must arrive as an empty batch, not as a timeout
	if o.accumulating {
		o.answerPending()
	}
	return nil
...
func (o *uriOwner) produce() error {
	...
	// the monitor below would fail with "unknown event" and say nothing about what was asked
...
func (o *uriOwner) readingFor(since string) ownerReadResponse {
	...
	// an empty list, never nil: rendered from nil it would be the literal null
	out.Value = nil
	out.Batches = []any{}
...
// a query carries text only and the builders read float64 and bool, so the conversion is by
// name: guessing from the value would turn a name of "123" into a number
...
var (
	...
	// comma separated in a uri: ?levels=warning,error
