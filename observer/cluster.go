...
type cluster struct {
	...
	// the inspect callback must not walk the store
	members int
...
func (c *cluster) Init(args ...any) error {
	...
	// sessions live under the manager, so process env does not reach them
...
func (c *cluster) startWatcher(node gen.Atom) {
	...
	if err != nil {
		// gen.ErrTaken means the previous watcher is still terminating
	...
	// monitor, not link: a dying watcher must not take the cluster actor with it
