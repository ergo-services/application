...
// a float because that is what a number sent by a client comes back as through JSON
...
// refused is what the gate already turned away before the stream existed
func newStreamRefusing(t *testing.T, refused map[string]string, raw ...string) *unit.Subject {
	...
	// a stream probes the node before it follows a lens on it, so every node named here has
	// to be one the network knows
...
// a stream carries notifications and, at the very end, the response to the listen request
// itself, so the frames are read as plain objects rather than as one shape
...
// empty for the response that closes the stream
...
func TestStreamFollowsAWarmResource(t *testing.T) {
	...
	// the frame is coalesced, so it is sent when the timer fires
	sub.FireTimers()
