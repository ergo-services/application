...
// the reading of a run, with the cursor the answer carries beside it
...
func TestClusterBatchAsksEachStepItsOwnQuestion(t *testing.T) {
	...
	// the whole answer of a fanout is a pointer, so the client gets it as a link and not only
	// as a uri buried in text
	...
	// the id is the caller's own and comes back, because answers arrive as they finish
	ids := map[string]bool{}
...
// since= takes what landed after the cursor the previous read handed out, and a cursor the
// ring no longer covers says so instead of quietly starting over
...
// Mode and State are enums, and the framework stopped writing their names when we took its
// json away: the view has to
...
// the same key is a retry only while it asks the same question: joining a run that answers
// something else would hand over answers nobody asked for
func TestARepeatedKeyJoinsOnlyTheSameQuestion(t *testing.T) {
	...
	// another question under a taken key: refused, and told where the run it collided with is
	...
	// retain is a preference, not the question
	if _, refusal := mcpCall(t, base, "cluster_query", map[string]any{
...
// a key names a reading with a cursor of its own, so without a caller to name it after every
// anonymous reader would share one position
...
func TestRunsAreCapped(t *testing.T) {
	...
	// the limit counts what is held, and a cancelled run is still held until it retires
...
// the whole path over real HTTP: a stream opened on a run is acknowledged with the run
// accepted, and the run moving reaches the client as a notification on that stream
