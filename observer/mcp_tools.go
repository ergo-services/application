...
var mcpTools = []mcpTool{
	...
	{
		Name:         "errors",
		Title:        "Registered wire errors",
		Action:       "errors",
		Capabilities: []string{inspect.CapGetErrors},
		Description: "The sentinel errors that node can carry over the network. A sentinel " +
			"missing here crosses the boundary as plain text, so an errors.Is against it " +
			"stops matching on the far side: that is the question this answers. The whole " +
			"dictionary comes back unless limit says otherwise, and it is process-wide, so " +
			"two nodes in one process answer the same. ID is this node's own cache id and " +
			"means nothing on another one, since each pair negotiates its own mapping at " +
			"handshake: compare by text.",
		Schema: mcpSchema{
			Type:     "object",
			Required: []string{nodeArgument},
			Properties: map[string]mcpSchemaItem{
				nodeArgument: nodeProperty(),
				"text": {Type: "string",
					Description: "Keep only errors whose text holds this."},
				"limit": {Type: "integer",
					Description: "Maximum errors to return, in cache order. Truncated says " +
						"how many were left out, and a truncated list cannot answer whether " +
						"a sentinel is absent: filter by text instead."},
			},
		},
	},
	{
		Name:         "atoms",
		Title:        "Registered atoms",
		Action:       "atoms",
		Capabilities: []string{inspect.CapGetAtoms},
		Description: "The atoms that node keeps in its wire-format cache, sent as a cache id " +
			"instead of a string. An atom missing here still travels, in full on every hop, " +
			"so absence is a frame-size cost and never a correctness problem. The cache is " +
			"process-wide, so two nodes in one process answer the same. ID is this node's " +
			"own cache id and means nothing on another one: compare by name.",
		Schema: mcpSchema{
			Type:     "object",
			Required: []string{nodeArgument},
			Properties: map[string]mcpSchemaItem{
				nodeArgument: nodeProperty(),
				"name": {Type: "string",
					Description: "Keep only atoms whose name holds this."},
				"limit": {Type: "integer",
					Description: "Maximum atoms to return, in cache order. Truncated says how " +
						"many were left out."},
			},
		},
	},
