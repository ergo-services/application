...
func TestSessionObservesRemoteNode(t *testing.T) {
	...
	// the watched event belongs to the remote node, not to this one
...
func TestSessionSwitchSurvivesStaleRelease(t *testing.T) {
	...
	// two frames after the release: the subscription outlived it
...
func TestAuthorizerGuardsListener(t *testing.T) {
	...
	// the guest holds the session id of root: the session is not theirs to use
...
func TestRateLimitOverHTTP(t *testing.T) {
	...
	// each surface meters on its own, so opening the stream leaves the POST budget whole
	b := openBrowser(t, port)
...
func TestStaleReleaseFromAnotherSession(t *testing.T) {
	...
	// the session before the reconnect: same node, same scope
	...
	// the cleanup of the dead session lands on the live one
...
// the metadata in the body and the mirroring headers over it, with the caller's own on top
...
func TestListenerSurfacesOverHTTP(t *testing.T) {
	...
	// a stream is the answer to a POST, so a GET is not a request here at all
	...
	// the handshake of the older revisions is not a method of this one
...
// two listeners: the endpoint is metered per listener while the secret belongs to one actor
...
func TestStatelessActionOverHTTP(t *testing.T) {
	...
	// string(): %q of a gen.Atom keeps the quotes Atom.String() adds
