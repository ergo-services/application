...
type wireRegisteredError struct {
	ID    uint16 `json:"i"`
	Text  string `json:"t"`
	Proto string `json:"p,omitempty"`
}

type wireErrorCache struct {
	Errors []wireRegisteredError `json:"e"`
}

func wireErrorsFrom(r inspect.ResponseGetErrors) wireErrorCache {
	out := wireErrorCache{Errors: make([]wireRegisteredError, 0, len(r.Errors))}
	for _, info := range r.Errors {
		out.Errors = append(out.Errors, wireRegisteredError{
			ID:    info.ID,
			Text:  info.Text,
			Proto: info.Proto,
		})
...
type wireRegisteredAtom struct {
	ID    uint16 `json:"i"`
	Name  string `json:"n"`
	Proto string `json:"p,omitempty"`
}

type wireAtomCache struct {
	Atoms []wireRegisteredAtom `json:"a"`
}

func wireAtomsFrom(r inspect.ResponseGetAtoms) wireAtomCache {
	out := wireAtomCache{Atoms: make([]wireRegisteredAtom, 0, len(r.Atoms))}
	for _, info := range r.Atoms {
		out.Atoms = append(out.Atoms, wireRegisteredAtom{
			ID:    info.ID,
