...
// the stage registrar answers ErrUnsupported to Nodes(), so a test runs in mesh mode unless
// it asks for RegistrarFull
type lens struct {
	t     *testing.T
	s     *stage.Stage
	gate  *stage.Node
...
// empty when the watcher is not running
...
func TestClusterKeepsUnreachableWhileReferenced(t *testing.T) {
	...
	// a hidden node dials out and runs no acceptor: nobody can dial it back
...
func TestClusterCentralRegistrar(t *testing.T) {
	...
	// no connection is established: the registrar is the only source
...
// a hidden node cannot model "listed by the registrar but unreachable": without an acceptor
// it has no routes, and a registrar rejects a registration without routes
...
func TestClusterWatchLimitFreesSlot(t *testing.T) {
	...
	// the gate watches itself, so one slot is taken from the start
	first := l.startPeer("first")
	l.connect(l.gate, first)
...
func TestClusterInfoRequest(t *testing.T) {
	...
	// a node that never answered has no peer list of its own: its edge is reported from the
	// other side, by the node that lists it
	edge := false
	for _, node := range info.Nodes {
