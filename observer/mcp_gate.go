...
func (g mcpGate) ServeHTTP(writer http.ResponseWriter, request *http.Request) {
	...
	// one byte past the limit, so an oversized body is not cut short and blamed for not
	// being JSON
	...
	// this revision defines no client notification over HTTP
	if envelope.ID == nil {
		g.fail(writer, request, nil, mcpInvalidRequest,
...
// Mcp-Param-* is deliberately not checked: those mirror only the parameters a tool marks with
// x-mcp-header, and no tool of ours marks any
func transportRefusal(request *http.Request, envelope mcpEnvelope) (int, string, any) {
	...
	// before the metadata is demanded: a client of an older revision does not know that _meta
	// carries the version
...
// a value in the sentinel is base64 of UTF-8 and has to be decoded before it is compared
