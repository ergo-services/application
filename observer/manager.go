...
// the SSE handler has ProcessPool: [managerName], so every SSE message comes here
type manager struct {
	act.Actor
...
func (m *manager) HandleMessage(from gen.PID, message any) error {
	...
	case sse.MessageConnect:
		// one meta, two surfaces: the path says which contract this stream speaks
...
func (m *manager) handleConnect(msg sse.MessageConnect) {
	...
	opts := gen.ProcessOptions{
		LinkParent: true,
		// explicit and well above 1: the remainder is truncated to whole seconds, so a
		// one-second budget can come out as zero and fail instantly
...
// the key is the whole idempotency: a registered name is claimed atomically, so a repeated
// call cannot start a second fan-out over the cluster
...
// counted, not tracked: a run retires on its own timer, and a counter kept here would drift
...
// the answer comes from the owner, so this callback does not wait for the observed node
