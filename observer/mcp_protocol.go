...
// the revision requires 404 with MethodNotFound and nothing else on this endpoint: it is what
// tells a client apart from a legacy server that does not host the modern one
...
// not configurable: it is how often the producers on an observed node republish. What a client
// may keep of the surface's own description is SurfaceMCP.CacheTTL.
