...
// stripped before canonicalization, so they never reach an owner's name
...
func parseURI(raw string) (mcpURI, error) {
	...
	// the observer's own resources carry no node
	switch segments[0] {
	case uriWordCluster:
		if len(segments) != 1 {
	...
	switch len(rest) {
	case 0:
		// ergo://node is the node lens itself
		out.Lens = "node"
	case 1:
		out.Lens = rest[0]
	case 2:
	...
	// one resource, one spelling: the owner is named after the canonical form
...
func (u mcpURI) validKey() error {
	...
	// a key of dots alone is a path a proxy would normalize
	if letters == 0 {
...
// sorted, so two spellings of one scope are one owner
