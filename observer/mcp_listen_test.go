...
// the envelope comes from the real transport gate: a hand-made one would test a request no
// client can send
...
func TestListenRefusesAnUnparsedRequest(t *testing.T) {
	...
	// the gate before this one is what parses, so an unparsed request is this surface failing
...
func TestListenNamesRefusalsCanonically(t *testing.T) {
	...
	// a string that never parsed has no canonical form and comes back as it went
